...
import { TodoRawData } from "./zustand/useTodo";


const App = () =>{
  const { todo, setTodo} = TodoRawData()
  ...
  useEffect(()=>{
    const TodoList = async() =>{
      try {
        ...
        setTodo(data.map((t) => NormalizeId(t)));
      }catch (error){
        setError(true)
        console.log(error)
      }
  ...
  const AddTask = async(e)=>{
        ...
        if(e.key === "Enter"){
          ...
          try {
            ...
            setTodo([NormalizeId(newtask), ...todo]);
            setInput("")
          }catch (error) {
  ...
  const handleDelete = async ()=>{
      const completedTasks = todo.filter(i => i.completed);
    ...
    try {
      ...
      const res = await fetch(`/todo/delete`, {method: "DELETE"});
      if(!res.ok){
        throw new Error("Failed to clear completed tasks")
      }
      setTodo(todo.filter(i => !i.completed))
    } catch (error) {
      console.log(error)
  ...
  const handleComplete = async (id)=>{
    ...
    if (!task){
      toast.warn("this error is case sentative rno");
      return ;
    } 
    ...
    try{
      const res = await fetch(`/todo/completed/${id}`, {
        method: "PUT", 
        headers: {"Content-Type" : "application/json"},
        body: JSON.stringify({completed: updatedtask})
      })
      if(!res.ok){
        throw new Error("Failed to Update Completed dem!")
      }
    }catch(error){
      console.log(error)
      setTodo(prevTodos)
  ...
  const handleSort = async()=>{
    ...
    const tasks = peopleClone.map((task, index) =>({
      ...task,
      orderIndex: index
    }))

    try {
      const res = await fetch("/todo/order", {
        method: "PUT",
        headers: {"Content-Type": "application/json"},
  ...
  return(
    <div className="w-full h-screen flex flex-col relative reduce">
      <div className={`flex-1 bg-no-repeat bg-cover bg-center transition-opacity duration-800 ease-in-out ${ theme === "light" ? "bg-[url(./assets/whitemood.png)] opacity-100" : "bg-[url(./assets//darkmood.png)] opacity-100"}`}></div>
          ...
          <div className={`flex-1 overflow-y-auto overflow-x-hidden scroll-auto flex flex-col w-full hideScrol`}>
            {error ? (
              ...
              (filteredTodos.map((item, i) => {
                const isLast = i === filteredTodos.length - 1;
                return (
                  <div 
                    key={item.id || i} 
                    className={`w-full gap-2.5 flex p-4 ${isLast ? "" : "border-b border-[#E3E4F1]"}`}
                    draggable
